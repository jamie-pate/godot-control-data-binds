...
func test_array_deep_mutated():
	...
	assert_eq(a.get_at(0).get_at(0), 1)
	var err = a.connect("deep_mutated", ss, "_on1")
	assert_eq(err, OK)
	err = a.connect("mutated", ss, "_on1")
	assert_eq(err, OK)
	a.get_at(0).set_at(0, 2)
	assert_eq(a.get_at(0).get_at(0), 2)
	...
	assert_eq(a.get_at(1).get_at(2), 3)
	a.get_at(1).set_at(2, 4)
...
func test_array_deep_mutated_deep():
	...
	assert_eq(a.get_at(0).get_at(1).get_at(2), 3)
	var err = a.connect("deep_mutated", ss, "_on1")
	assert_eq(err, OK)
	a.get_at(0).get_at(1).set_at(2, -1)
	assert_eq(a.get_at(0).get_at(1).get_at(2), -1)
...
func test_model_array_deep_mutated():
	...
	assert_eq(m.value.get_at(0), 0)
	var err = m.connect("deep_mutated", ss, "_on1")
	assert_eq(err, OK)
	m.value.set_at(0, -1)
	assert_eq(m.value.get_at(0), -1)
...
func test_array_model_deep_mutated():
	...
	assert_eq(a.get_at(0).value, 0)
	var err = a.connect("deep_mutated", ss, "_on1")
	assert_eq(err, OK)
	a.get_at(0).value = -1
	assert_eq(a.get_at(0).value, -1)
